...
The type of the argument passed to `Provider.filter` is changed from `Predicate` to `Spec` to have a more consistent API.
This change should not affect anyone using `Provider.filter` with a lambda expression.
However, this might have consequences for plugin authors.
