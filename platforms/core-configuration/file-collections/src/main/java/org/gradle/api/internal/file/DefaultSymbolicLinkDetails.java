...
public class DefaultSymbolicLinkDetails implements SymbolicLinkDetails {
    ...
    public DefaultSymbolicLinkDetails(Path path, RelativePath relativePath) {
        ...
        if (target.isAbsolute()) {
            ...
            boolean _isRelative;
            Path _absoluteTarget;
            try {
                _absoluteTarget = resolvedTarget.toRealPath();
                _isRelative = isRelativeToRoot(path, relativePath, resolvedTarget, _absoluteTarget);
            } catch (IOException e) {
                _absoluteTarget = resolvedTarget;
                _isRelative = false;
            }
            absoluteTarget = _absoluteTarget;
